...
the type is templated and made static, it is essentially remembered for the life of the programming.
Thus whenever a templated type is invoked, which already exists, the pointer to that instance is returned instead.
...
\end{lstlisting}
The event enum class, acts a way to indicate what events is being passed around. 
...
be overriden by any class that inherits state. That and the two other functions onEntry and onExit,
make up the three functions that the statemachine uses to transfer control. It is also what enacts
the GOF state pattern, because it creates a new class with defined functions when the states is changed.
...
Due to the way the classes, or this case, states are implemented, all states comply with the interface.
...
\begin{lstlisting}[language=C++, caption={Operational state class declaration}, firstnumber=580]
...
\begin{lstlisting}[language=C++, caption={Operational state class definition, partly}, firstnumber=780]
...
The Operational class, which acts the main driving state for the outer statemachinem, aswell as the
...
\begin{lstlisting}[language=C++, caption={EmbeddedSystemX}, firstnumber=1128]
...
The scheduler itself owns the ActivationQueue and all the requests are a inherited request type.
...
there is work to be done. This newly spawned thread will the take the work from the queue using .remove() and it will spawn
...
This also fullfills the promise and the proxy is delivered the future. though it is void,
